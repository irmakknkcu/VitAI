...
async function getProfileContext(db, userId) {
    const [profiles] = await db.query(
        `SELECT u.name, u.surname, p.age, p.gender, p.height, p.weight, p.goal, p.activity,
                ...
                p.daily_calories_taken, p.daily_calories_goal
    ...
    // Tahlil verileri (sadece 0'dan büyük değerleri ekle)
    const labParts = [];
    if (Number(p.hemoglobin) > 0)  labParts.push(`Hemoglobin: ${p.hemoglobin} g/dL`);
    if (Number(p.glucose) > 0)     labParts.push(`Glucose: ${p.glucose} mg/dL`);
    if (Number(p.cholesterol) > 0) labParts.push(`Cholesterol: ${p.cholesterol} mg/dL`);
    if (Number(p.vitamin_d) > 0)   labParts.push(`Vitamin D: ${p.vitamin_d} ng/mL`);
    const labContext = labParts.length > 0 ? ` | Lab results: ${labParts.join(', ')}` : '';

    // Günlük kalori bilgisi
    const caloriesTaken = Number(p.daily_calories_taken) || 0;
    const caloriesGoal = Number(p.daily_calories_goal) || (tdee !== 'N/A' ? Number(tdee) : 0);
    const calorieContext = caloriesGoal > 0
        ? ` | Today's calories: ${caloriesTaken} kcal consumed / ${caloriesGoal} kcal goal`
        : '';
...
router.post('/chat', auth, async (req, res) => {
    try {
        ...
        const systemPrompt = `You are VitAI Diet Coach, a professional AI dietitian and health coach.
...
${profileContext}`.replace(/\s+/g, ' ').trim();
